...
export function MorphingCardStack({
  ...
  if ((!cards || cards.length === 0) && !children) {
    return null
  }
  ...
  const containerStyles = {
    stack: "relative h-[500px] w-full max-w-2xl",
...
interface MorphingCardProps extends React.HTMLAttributes<HTMLDivElement> {
  children: React.ReactNode
}

export function MorphingCard({ children, className, ...props }: MorphingCardProps) {
  return (
    <div
      className={cn(
        "rounded-xl border bg-card p-6 shadow-sm h-full",
        "bg-radial-gradient",
        className
      )}
      {...props}
