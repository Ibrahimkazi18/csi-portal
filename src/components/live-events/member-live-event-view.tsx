...
import { 
  ...
  CheckCircle, Clock, Star, TrendingUp, RefreshCw
} from "lucide-react"

interface MemberLiveEventViewProps {
  eventData: any
  userStatus: any
  onRefresh: () => void
}

export function MemberLiveEventView({ eventData, userStatus, onRefresh }: MemberLiveEventViewProps) {
  ...
  const getParticipantsInRound = (roundId: string | null) => {
    if (roundId === null) {
      ...
      const participantsInRounds = progress.map((p: any) => p.team_id || p.user_id)
      return progress.filter((p: any) => !participantsInRounds.includes(p.team_id || p.user_id))
    }

    if (roundId === 'eliminated') {
      return progress.filter((p: any) => p.eliminated)
    }

    return progress.filter((p: any) => p.round_id === roundId && !p.eliminated)
  }

  const getUserCurrentRound = () => {
    if (!userStatus?.currentProgress) return null
    return rounds.find((r: any) => r.id === userStatus.currentProgress.round_id)
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <h1 className="text-3xl font-bold text-neon text-lavender">
            {isCompleted ? "Event Results" : "Live Event"}
          </h1>
          {event.is_tournament && (
      ...
      <div className="overflow-x-auto">
        <div className="flex gap-4 min-w-max pb-4">
          ...
          {rounds.map((round: any) => (
            <Card key={round.id} className="min-w-[280px] h-[400px] flex flex-col bg-dark-surface border-border">
              ...
              <CardContent className="flex-1 overflow-y-auto space-y-2">
                ...
                {getParticipantsInRound(round.id).length === 0 && (
          ...
          {/* Winners Column */}
          {winners.length > 0 && (
            <Card className="min-w-[280px] h-[400px] flex flex-col bg-dark-surface border-yellow-500/30 glow-yellow">
              <CardHeader className="pb-3">
                <CardTitle className="text-sm flex items-center gap-2">
          ...
          {/* Eliminated Column */}
          <Card className="min-w-[280px] h-[400px] flex flex-col bg-dark-surface border-red-500/30">
            <CardHeader className="pb-3">
              <CardTitle className="text-sm flex items-center gap-2">
                <X className="h-4 w-4 text-red-400" />
                Eliminated ({getParticipantsInRound('eliminated').length})
              </CardTitle>
            </CardHeader>
            <CardContent className="flex-1 overflow-y-auto space-y-2">
              {getParticipantsInRound('eliminated').map((participant: any) => (
                <ParticipantCard key={participant.id} participant={participant} showDetails={true} />
              ))}
              {getParticipantsInRound('eliminated').length === 0 && (
                <div className="text-center text-muted-foreground text-sm py-8">
                  No eliminations
                </div>
              )}
